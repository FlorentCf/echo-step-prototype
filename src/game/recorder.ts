...
export function startRecording(player: PlayerState): Recording {
  return {
    snapshots: [snapshotFromPlayer(player, 0)],
  };
}

export function recordFrame(recording: Recording, player: PlayerState, t: number): void {
  ...
  if (last && Math.abs(last.t - roundedT) < FIXED_DT * 0.4) {
    recording.snapshots[recording.snapshots.length - 1] = snapshotFromPlayer(player, roundedT);
    return;
  }

  recording.snapshots.push(snapshotFromPlayer(player, roundedT));
}
...
export function finalizeRecording(
  ...
  const finalPose = mode === "dead" ? deadSnapshot(player, currentFrame * FIXED_DT) : standingSnapshot(player, currentFrame * FIXED_DT);
  ...
  for (let frame = 0; frame <= frameCount; frame += 1) {
    if (!snapshots[frame]) {
      ...
      snapshots[frame] = { ...previous, t: roundTime(frame * FIXED_DT) };
    }
  }
  ...
  return snapshots.slice(0, frameCount + 1);
}
...
export function snapshotFromPlayer(player: PlayerState, t: number): PlayerSnapshot {
  return {
    t: roundTime(t),
    x: player.x,
    y: player.y,
...
function standingSnapshot(player: PlayerState, t: number): PlayerSnapshot {
  return {
    ...snapshotFromPlayer(player, t),
    vx: 0,
    vy: 0,
    alive: true,
  };
